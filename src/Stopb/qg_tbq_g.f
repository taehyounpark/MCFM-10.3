...
c---  calculate the qg and qbarg matrix element first
               call inter(p,1,2,3,4,5,6,mt,mb,msq_qg)
               call inter(p,5,2,3,4,1,6,mt,mb,msq_qbarg)
c---  now calculate the gq and gqbar matrix element
               call inter(p,2,1,3,4,5,6,mt,mb,msq_gq)
               call inter(p,5,1,3,4,2,6,mt,mb,msq_gqbar)
            else
c---  calculate the qg and qbarg matrix element first
               call inter(p,1,2,4,3,5,6,mb,mt,msq_qg)
               call inter(p,5,2,4,3,1,6,mb,mt,msq_qbarg)
c---  now calculate the gq and gqbar matrix element
               call inter(p,2,1,4,3,5,6,mb,mt,msq_gq)
               call inter(p,5,1,4,3,2,6,mb,mt,msq_gqbar)
            endif
         endif
...
c--- between the two configurations
            call inter_gg(p,msq_gg_a,msq_gg_b,msq_gg_intf)
         endif
      endif
...
c--- the matrix elements with an extra quark line:
         if (nwz == +1) then
            call inter_qq(p,1,2,3,4,5,6,mt,mb,msq_qq12,msq_qq12_intf)
            call inter_qq(p,5,6,3,4,1,2,mt,mb,msq_qq56,msq_qq56_intf)
         else
            call inter_qq(p,1,2,4,3,5,6,mb,mt,msq_qq12,msq_qq12_intf)
            call inter_qq(p,5,6,4,3,1,2,mb,mt,msq_qq56,msq_qq56_intf)
         endif
      endif
