...
c      j34 is flavour of 34 line
     ...
     & amp(2,2,2,2),propL(2,2),propR(2,2),prop
      ...
      p1(:)=cmplx(p(i1,:),kind=dp)
      p2(:)=cmplx(p(i2,:),kind=dp)
      p3(:)=cmplx(p(i3,:),kind=dp)
      p4(:)=cmplx(p(i4,:),kind=dp)
      p123(:)=cmplx(p(i1,:)+p(i2,:)+p(i3,:),kind=dp)
      p124(:)=cmplx(p(i1,:)+p(i2,:)+p(i4,:),kind=dp)
      p134(:)=cmplx(p(i1,:)+p(i3,:)+p(i4,:),kind=dp)
      p234(:)=cmplx(p(i2,:)+p(i3,:)+p(i4,:),kind=dp)
      p12(:)=cmplx(p(i1,:)+p(i2,:),kind=dp)
      p34(:)=cmplx(p(i3,:)+p(i4,:),kind=dp)
      p56(:)=cmplx(p(i5,:)+p(i6,:),kind=dp)
      s12=real(p12(4)**2-p12(1)**2-p12(2)**2-p12(3)**2)
      s34=real(p34(4)**2-p34(1)**2-p34(2)**2-p34(3)**2)
      s56=real(p56(4)**2-p56(1)**2-p56(2)**2-p56(3)**2)
      prop=s56/cmplx(s56-zmass**2,zmass*zwidth,kind=dp)
...
c---first index is flavour, second is helicity of 56 line
      ...
      p5(:)=cmplx(p(q5,:),kind=dp)
      p6(:)=cmplx(p(q6,:),kind=dp)
