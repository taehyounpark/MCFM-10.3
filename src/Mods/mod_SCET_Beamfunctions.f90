...
module SCET_Beamfunctions
    use types
    implicit none

    public :: xbeam1bis_new, xbeam2bis_new

    private

    contains

    subroutine xbeam1bis_new(ih,zin,xb,QB,btau, ibeam_in, scale_in, facscale_in, disttau)
    use LHAPDF
    implicit none
    include 'types.f'
    include 'constants.f'
    ...
    real(dp), intent(in), optional :: scale_in, facscale_in
    ...
    if (present(scale_in) .AND. present(facscale_in)) then
        scale_used = scale_in
        facscale_used = facscale_in
    else
        scale_used = scale
...
!--- changing variables so z integral ranges from xb to 1
    zb = (one - xb)*zin + xb
    jaco = abs(one - xb)
...
! calculate quark+antiquark sum
    ...
    !--- rather than log(taucut*QB/musq) => extra +log(QB/scale)
        ...
        !--- adjust normalization to (as/2/pi) rather than (as/4/pi)
            btau(j,:)=btau(j,:)/two
        endif

    !      Lt1cut = log(taucut*QB/musq)
    !      Lt1cut2 = Lt1cut**2/2.0_dp
    !      beam1(j)=half*(btau(-1)+btau(0)*Lt1cut+btau(1)*Lt1cut2)
    enddo
    return
    end subroutine xbeam1bis_new

    subroutine xbeam2bis_new(ih,zin,xb,QB,btau, ibeam_in, scale_in, facscale_in, disttau)
    use LHAPDF
    implicit none
    include 'types.f'
    include 'constants.f'
...
! calculate quark+antiquark sum
    ...
    do j=-nf,nf
        ...
        !---additional factorization scale dependence in 1505.04794, Eq. (A.20)
            ...
            +four*Gag0*p0ijterm*log(scale_used/facscale_used)
            ...
            +(-gBg0*p0ijterm+four*p0ikp0kjterm)*two*log(scale_used/facscale_used)
            btau(j,-1)=four*I2ijterm &
            +four*(I1ikp0kjterm+p1ijterm)*two*log(scale_used/facscale_used) &
            +(be0*p0ijterm+two*p0ikp0kjterm)*four*log(scale_used/facscale_used)**2
        ...
        !---additional factorization scale dependence in 1505.04794, Eq. (A.20)
            ...
            +four*Gaq0*p0ijterm*log(scale_used/facscale_used)
            btau(j,0)=(Gaq0**2*zeta3+Gaq0*gBq0*pisq/twelve-gBq1/two)*fx0(j) &
            -Gaq0*pisq/three*p0ijterm-(gBq0+two*be0)*I1ijterm &
            +four*(I1ikp0kjterm+p1ijterm) &
            +(-gBq0*p0ijterm+four*p0ikp0kjterm)*two*log(scale_used/facscale_used)
            btau(j,-1)=four*I2ijterm &
            +four*(I1ikp0kjterm+p1ijterm)*two*log(scale_used/facscale_used) &
            +(be0*p0ijterm+two*p0ikp0kjterm)*four*log(scale_used/facscale_used)**2
    ...
    !--- rather than log(taucut*QB/musq) => extra +log(QB/scale)
        ...
        !--- adjust normalization to (as/2/pi)^2 rather than (as/4/pi)^2
            btau(j,:)=btau(j,:)/four
        endif
    ...
    !      Lt1cut = log(taucut*QB/musq)
    !      Lt1cut2 = Lt1cut**2/2.0_dp
    !      Lt1cut3 = Lt1cut**3/3.0_dp
    !      Lt1cut4 = Lt1cut**4/4.0_dp

    !      beam2(j)=btau(-1)+btau(0)*Lt1cut+btau(1)*Lt1cut2
    !     &        +btau(2)*Lt1cut3+btau(3)*Lt1cut4
    enddo
