...
!$omp end master
 ...
 42   continue
      if (verbose .and. rank == 0 .and. first) then
       write(6,*)
       write(6,*) '*************************** f(p1)+f(p2) --> *****'//
     &           '*************************************'
      j=index(pname,'[')
...
c-- Default behaviour: the same value as for the other jets
      ...
      onlyaxial=.false.      ! only compute axial (t,b) loops
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc == 2) .or. (nproc == 7)) then
        kcase=kWln_ew
        mass3=wmass
        width3=wwidth
        n3=1
...
c---W^+
        if     (nproc == 2) then
c-- 2  '  f(p1)+f(p2) --> W^+(-->nu(p3)+e^+(p4)) [EW + gamma(p5)]'
          plabel(3)='nl'
          plabel(4)='ea'
          plabel(5)='ga'
          nwz=1
c---W^-
        elseif (nproc == 7) then
c-- 7  '  f(p1)+f(p2) --> W^-(-->e^-(p3)+nu~(p4)) [EW + gamma(p5)]'
          plabel(3)='el'
          plabel(4)='na'
          plabel(5)='ga'
          nwz=-1
      ...
      elseif ((nproc == 3) .or. (nproc == 8)) then
        kcase=kWln_aq
        mass3=wmass
        width3=wwidth
        n3=1
...
c---W^+
        if     (nproc == 3) then
c-- 3  '  f(p1)+a(p2) --> W^+(-->nu(p3)+e^+(p4)) [EW photon-induced + q(p5)]' 'N'
          plabel(3)='nl'
          plabel(4)='ea'
          plabel(5)='pp'
          nwz=1
c---W^-
        elseif (nproc == 8) then
c-- 8  '  f(p1)+a(p2) --> W^-(-->e^-(p3)+nu~(p4)) [EW photon-induced + q(p5)]' 'N'
          plabel(3)='el'
          plabel(4)='na'
          plabel(5)='pp'
          nwz=-1
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc > 30) .and. (nproc <= 35)) then
        ...
        if (dosingcheck .eqv. .false.) new_pspace = .false.
...
c-----------------------------------------------------------------------
      ...
      elseif (nproc/10 == 6) then
        kcase=kWWqqbr
        nflav=4 ! avoid contamination from top-quark processes, W+ W- b b~
        call readcoup
        nqcdjets=0
        plabel(7)='pp'
        nwz=1
...
c--     '  f(p1)+f(p2) --> W^+ + W^- + f(p7) (for total Xsect)' (removebr=.true.)
          kcase=kWW_jet
          nflav=4 ! avoid contamination from top-quark processes, W+ W- b b~
          if (useblha == 1) nflav=5 ! OpenLoops/Recola comparison only for nf=5
          nqcdjets=1
          ndim=13
          plabel(3)='nl'
          plabel(4)='ma'
...
c-----------------------------------------------------------------------
      ...
      elseif (((nproc >= 70) .and. (nproc <= 80))
     &       .or. (nproc == 711) .or. (nproc == 761))  then
        kcase=kWZbbar
...
c        endif
        ...
        if ((nproc <= 75) .or. (nproc == 711)) then
c-- W^+Z
          nwz=+1

          if     ((nproc == 71) .or. (nproc == 711)) then
...
c--     '  f(p1)+f(p2) --> W^+ (for total Xsect) + Z^0 ' (removebr=.true.)
            ...
            plabel(5)='ml'
            plabel(6)='ma'
c identical-particle process (all electrons)
c leave particle labels as above to aid cuts on W and Z decay products
            if (nproc == 711) then
              interference=.true.
              vsymfact=0.5_dp
            endif
...
c--  75 '  f(p1)+f(p2) --> W^+(-->nu(p3)+mu^+(p4))+Z^0(-->2*(u(p5)+u~(p6)))'
        ...
        elseif ((nproc >= 76)  .or. (nproc == 761)) then
c-- W^-Z
          nwz=-1

          if     ((nproc == 76)  .or. (nproc == 761)) then
...
c-----------------------------------------------------------------------
      ...
      elseif (((nproc > 80) .and. (nproc <= 90)) .or. (nproc == 8211)) then
        kcase=kZZlept
        call checkminzmass(1)
...
c--- check runstring to change from (e,mu) Z decays to (e,e) or (mu,mu)
          if     (index(runstring,'ELEL') > 0) then
            ...
            interference=.true.
            vsymfact=0.25_dp
          elseif (index(runstring,'MUMU') > 0) then
            plabel(3)='ml'
            plabel(4)='ma'
            interference=.true.
            vsymfact=0.25_dp
...
c Remove virtual photon
            q1=0._dp
            q2=0._dp
            rq1=0._dp
            rq2=0._dp
c Adjust BR for use of CMS
            BrnRat=2._dp*(brzee*(abs(zle)**2+abs(zre)**2)/(le**2+re**2))**2  ! factor of 2 for identical particles
          endif
...
c--  87 '  f(p1)+f(p2) --> Z^0(-->e^-(p3)+e^+(p4)) + Z^0(-->3*(nu(p5)+nu~(p6))) [no gamma^*]'
          ...
          plabel(6)='na'
          q2=zip
c no more factors of sqrt(3._dp) here, c.f. rescale in qqb_zz.f etc.
          l2=ln
          r2=rn
          rq2=zip
          zl2=zln
          zr2=zrn
        elseif (nproc == 8211) then
c--  82 '  f(p1)+f(p2) --> Z^0(-->e^-(p3)+e^+(p4)) + Z^0(-->3*(nu(p5)+nu~(p6)))'
c--  87 '  f(p1)+f(p2) --> Z^0(-->e^-(p3)+e^+(p4)) + Z^0(-->3*(nu(p5)+nu~(p6))) [no gamma^*]'
          kcase=kVVlept
          nflav=4 ! avoid contamination from top-quark processes, W+ W- b b~
...
c-----------------------------------------------------------------------
      ...
      elseif (nproc == 900) then
        ...
        mass3=wmass
        width3=wwidth
        CKMdiag = .true.        ! required for this process

        mcfmplotinfo= (/ 34, 56, 3456, (0,j=1,47) /)

c--  900 '  f(p1)+f(p2) --> (W+ + W-) H(-->tau-(p5)+tau+(p6)) '
c--      '  f(p1)+f(p2) --> W^+H + W^-H (for total Xsect)' (removebr=.true.)
        plabel(3)='nl'
        plabel(4)='ea'
        nwz=2
        if (removebr) then
...
c-----------------------------------------------------------------------
        ...
        elseif     (nproc == 1111) then
c-- 1111 '  f(p1)+f(p2) --> H(-->b(p3)+bbar(p4))incl scalar loops'
c--      '  f(p1)+f(p2) --> H (for total Xsect)incl scalar loops' (removebr=.true.)
        kcase=kggscl0
        call sethparams(br,wwbr,zzbr,tautaubr,gamgambr,zgambr)
        plabel(5)='pp'
        ndim=4
        ...
        stop
c!$omp master
c        if (rank == 0 .and. first) then
c          write(6,*) 'mscalar=',mscalar
c          write(6,*) 'DeltaM=',DeltaM
c          write(6,*) 'theta/pi=',theta/pi
c          write(6,*) 'tanBeta=',tanBeta
c        endif
c!$omp end master
c        call fill_mloopsq_gloop()
...
c-----------------------------------------------------------------------
     ...
     &       ((kpart==kvirt) .or. (kpart==ktota)) ) then
        ...
        if ( (nproc == 138) .and. (kpart /= kreal) ) then
...
c--  157 '  f(p1)+f(p2) --> t t~ (for total Xsect)'
        ...
        if (kewcorr == kexact) then
          ...
          tevscale = 1._dp ! this is abused in EW corrections as rescaling of top Yukawa
        endif
...
c-----------------------------------------------------------------------
      elseif (nproc == 164 .or. nproc == 169) then
        ...
        if ((origKpart /= klord) .and. (mode_anomcoup .neqv. .false.)) then
            write (*,*) "Anomalous couplings mode only supported at LO!"
            write (*,*) "In this mode 1/Lambda^8 contributions are included"
            write (*,*) "and one can directly compare to anomalous couplings"
            write (*,*) "results."
            write (*,*) ""
            write (*,*) "For Lambda=1 the following equalities hold:"
            write (*,*) "deltaVL = C_phiQ (SM is included)"
            write (*,*) "VR = conjg(C_phiphi)"
            write (*,*) "gR = -4*wmass*C_tW"
            write (*,*) "gL = -4*wmass*conjg(C_dW)"
            write (*,*) ""
            write(6,*) 'Abort in chooser'
            stop
        endif

        ! enforce diagonal CKM matrix
        Vud = 1._dp
        Vus = 0._dp
        Vub = 0._dp
        Vcd = 0._dp
...
c       ! we enforce some parameters here to improve stability
c       cutoff_s = 0.001_dp
c       aii = 0.05_dp
c       aif = 0.05_dp
c       afi = 0.05_dp
        ...
        ! for anti-top production we need to complex conjugate all
        ! wilson coefficients due to the way we cross particles
        ! first check required, otherwise signs will be flipped
        ! after every call
        if (nproc == 169 .and. first) then
            call anomcoup_cc()
        endif

        ! if DDIS is enabled we need to initialize the scale for the
        ! decay width
        if (use_DDIS) then
            call set_scales(mt,mt)
        endif
        twidth = singletop2_decaywidth()

        if ((rank == 0) .and. first) then
            write (*,*) ""
            if (origKpart == klord) then
                if (enable_lambda2) then
                    write (*,'(A,F8.3)') " * Using EFT adjusted top width (LO) ", twidth
                else
                    write (*,'(A,F8.4)') " * Using top width (LO) ", twidth
                endif
            else
                if (enable_lambda2) then
                    write (*,'(A,F8.3)') " * Using EFT adjusted top width (NLO) ", twidth
                else
                    write (*,'(A,F8.4)') " * Using top width (NLO) ", twidth
                endif
            endif
        endif
        ...
        if (doscalevar) then
          write (*,*) "scale variation currently not supported"
          write (*,*) "please contact authors."
          write(6,*) 'Abort in chooser'
          stop
        endif

        if (doPDFerrors) then
            write (*,*) "PDF errors currently not supported"
            write(6,*) 'Abort in chooser'
      stop
        endif

        if ((jetalgorithm /= kt)
     &  .and. (jetalgorithm /= antikt)
     &  .and. (jetalgorithm /= noclustering)
     &  .and. (jetalgorithm /= 0)) then
            write (*,*) "Unsupported jet algorithm for this process"
            write (*,*) "jetalgorithm=",jetalgorithm
            write(6,*) "Abort in chooser"
            stop
        endif
        ...
        if (nproc == 164) then
            ...
            plabel(4)='ea'
            plabel(5)='bq'
        elseif (nproc == 169) then
            nwz=-1
            plabel(3)='el'
            plabel(4)='na'
            plabel(5)='ba'
        else
            stop 'Abort in chooser'
        endif
        ...
        mb=zip
        n3=1 ! for phase space generation, generate W breit wigner
        mass2=mt
        width2=twidth
        mass3=wmass
        width3=wwidth
        ...
        if (removebr .or. zerowidth) then
          write (*,*) "WARNING: removebr = .true. and zerowidth = .true. not supported. Setting to .false."
          removebr = .false.
          zerowidth = .false.
        endif
...
c--- ndim is one less than usual, since the top is always on-shell
        ...
        kcase=kbq_tpq
        zerowidth = .true.
        ! for preliminary results we do not include any partons
        ! from other ipsgen pieces in the jet tagging or counting
        ...
        elseif (currentContrib == 4) then
            if (kpart == kreal) then
                if (ipsgen == 4) then ! RR
                    ...
                    ! this is a double real contribution
                    ndim = 12
                elseif (ipsgen == 5) then ! RV
                    nqcdjets = 1
                    notag = 1
                    plabel(3)='nl'
                    plabel(4)='ea'
                    ...
                    ! this is a single real contribution
                    ! additional two since this is a below cut piece
                    ndim = 9 + 2

                else
                    error stop "ipsgen: undefined interference contribution"
                endif
            elseif (kpart == kvirt) then ! VR
                if (ipsgen == 4) then
                    nqcdjets = 1
                    notag = 1
                    plabel(3)='nl'
                ...
                error stop "kpart: undefined interference contribution"
            endif
        ...
        elseif (currentContrib == 6) then
            ...
            ! pass for first call to chooser in parseinput (when ipsgen=0)
            continue
        endif
        ...
        ! let's try and not depend on these
        n3=1
        mass2=mt
        width2=twidth
        mass3=wmass
...
c--      '  f(p1)+f(p2) --> t(no BR) + q(p6)' (removebr=.true.)
        kcase=kbq_tpq
        new_pspace = .false.
        ! new_pspace works in principle but seems less efficient
        isub=1
        nqcdjets=2
        plabel(3)='nl'
        plabel(4)='ea'
...
c--- ndim is one less than usual, since the top is always on-shell
        ndim=9
        mb=zip
        if (debug) write(6,*) 'Enforcing mb=0 for this process!'
        n3=1
        mass2=mt
        width2=twidth
        mass3=wmass
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc  ==  190) .or. (nproc  ==  191)) then
...
c--  2011'  f(p1)+f(p2)--> H(-->b(p3)+b~(p4)) + f(p5) [scalar]'
c--      '  f(p1)+f(p2)--> H(p3+p4) + f(p5) (scalar)' (removebr=.true.)
        hdecaymode='bqba'
        plabel(3)='bq'
        plabel(4)='ba'
        nqcdjets=3
...
c        write(6,*) 'Reading squark parameters from squark.dat'
c        open(unit=55,file='squark.dat',status='unknown')
c        read(55,*) mscalar
c        read(55,*) DeltaM
c        read(55,*) theta
c        read(55,*) tanBeta
...
! default alpha-cut values are especially bad, fix that here
       if (aif == 1._dp) aif=0.1_dp
       if (afi == 1._dp) afi=0.1_dp
...
c-- 251 '  f(p1)+f(p2) --> W^+(-->nu(p3)+e^+(p4)) + W^+(-->nu(p5)+e^+(p6))+f(p7)+f(p8)'
      ...
      removebr = .false.
      if ((rank == 0) .and. first) then
        write(*,*) 'Setting zerowidth to true for process 251'
        write(*,*) 'Setting removebr to false for process 251'
      endif
...
c-- 252 '  f(p1)+f(p2) --> W^+(-->nu(p3)+e^+(p4)) + W^+(-->nu(p5)+e^+(p6))+f(p7)+f(p8)+f(p9)'
      ...
      removebr = .false.
      if ((rank == 0) .and. first) then
        write(*,*) 'Setting zerowidth to true for process 252'
        write(*,*) 'Setting removebr to false for process 252'
      endif
...
c--     '  f(p1)+f(p2) --> H(no BR)+f(p5)+f(p6)[in heavy top limit]' (removebr=.true.)
        ...
        elseif     (nproc == 2691) then
c-- 2691 ' f(p1)+f(p2) --> H(gamma(p3)+gamma(p4))+f(p5)+f(p6)[scalar]'
c--     '  f(p1)+f(p2) --> H(no BR)+f(p5)+f(p6)[scalar]' (removebr=.true.)
          hdecaymode='gaga'
          plabel(3)='ga'
          plabel(4)='ga'
          kcase=kh2jscl
...
c-----------------------------------------------------------------------
      ...
      elseif (nproc == 280) then
             ...
             inclusive=.true.
c           write(6,*)
c           write(6,*) 'Setting inclusive = .true. for direct photon production.'
...
c==========reset esq^2 so alpha_EM  = 1/137
             reset_alphaEM=.true.
             ndim=4
             kcase=kgamgam
             plabel(3)='ga'
...
c-- parameters for phase space, to account for off-shell W
...
c--- total cross-section
        if (removebr) then
          ...
          BrnRat=brwen
c Adjust BR for use of CMS
          BrnRat=brwen*xw/abs(zxw)
        endif

c-----------------------------------------------------------------------

      elseif ((nproc == 291) .or. (nproc == 296)) then
        kcase=kWga_ew
        nqcdjets=0
        ndim=7
        mb=0
        ...
        if     (nproc == 291) then
c-- 291 '  f(p1)+f(p2) --> W^+(-->nu(p3)+e^+(p4))+gamma(p5)'
c--     '  f(p1)+f(p2) --> W^+ (no BR) + gamma(p5)' (removebr=.true.)
          nwz=1
          plabel(3)='nl'
          plabel(4)='ea'
        elseif (nproc == 296) then
c-- 296 '  f(p1)+f(p2) --> W^-(-->e^-(p3)+nu~+(p4))+gamma(p5)'
c--     '  f(p1)+f(p2) --> W^- (no BR) + gamma(p5)' (removebr=.true.)
          nwz=-1
          plabel(3)='el'
          plabel(4)='na'
...
c-- parameters for phase space, to account for off-shell W
         if (zerowidth .eqv. .false.) then
           doipsgen=.true.
c           maxipsgen=2 ! set in mcfm_vegas_adaptive.f
!$omp master
           if (rank == 0 .and. first) then
           write(6,*)
           write(6,*) 'Setting removebr to .false. in order to ensure'
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc == 292) .or. (nproc == 297)
     &   .or. (nproc == 2921).or. (nproc == 2971)) then
...
c           maxipsgen=2 ! set in mcfm_vegas_adaptive.f
        ...
        if     ((nproc == 292) .or. (nproc  ==  2921)) then
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc == 294) .or. (nproc == 299)) then
...
c-- parameters for phase space, to account for off-shell W
        ...
        if     (nproc == 294) then
c 292 '  f(p1)+a(p2) --> W^+(-->nu(p3)+e^+(p4))+gamma(p5)+f(p6)'
c     '  f(p1)+a(p2) --> W^+(No BR)+gamma(p5)+f(p6) (removebr =.true.)'
           nwz=1
           plabel(3)='nl'
           plabel(4)='ea'

        elseif (nproc == 299) then
c 297 '  f(p1)+a(p2) --> W^-(-->e^-(p3)+nu~(p4))+gamma(p5)+f(p6)'
c     '  f(p1)+a(p2) --> W^-(no BR)+gamma(p5)+f(p6) (removebr=.true.)'
           nwz=-1
           plabel(3)='el'
           plabel(4)='na'
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc == 2941) .or. (nproc == 2991)) then
...
c--------------------------------------------------------------------------------------------------
        ...
        elseif ((nproc == 300) .or. (nproc == 305) .or. (nproc ==3000)) then
          kcase=kZgamma
          new_pspace = .true.
          nqcdjets=0
          ndim=7
...
c--     '  f(p1)+f(p2) --> Z^0 (no BR) +gamma(p5)' (removebr=.true.)
            ...
            plabel(3)='el'
            plabel(4)='ea'
            call setDecayChannel(decayElAntiEl)
            q1=-1._dp
            l1=le
            r1=re
            zl1=zle
...
c-- 305 '  f(p1)+f(p2) --> Z^0(-->3*(nu(p3)+nu~(p4)))-(sum over 3 nu)+gamma(p5)'
            plabel(3)='nl'
            plabel(4)='na'
            call setDecayChannel(decayNuAntiNu)
            q1=zip
            l1=ln*sqrt(3._dp)
            r1=rn*sqrt(3._dp)
            zl1=zln*sqrt(3._dp)
...
c-- 3000 '  f(p1)+f(p2) --> Z(-->3*(d(p3)+d~(p4)))+gamma(p5)'
            !plabel(3)='bq'
            !plabel(4)='ba'
            ! handle them for all purposes like leptons, but only for the
            ! matrix elements like quarks through setDecayChannel
            plabel(3)='el'
            plabel(4)='ea'
            call setDecayChannel(decayQuarks)
            if (removebr) then
              plabel(3)='ig'
              plabel(4)='ig'
              BrnRat=branch_zqq()
            endif
            ! can uncomment to ensure q1,l1,r1 are unused
            q1=0d0 !q1=ieee_value(0._dp, ieee_quiet_nan)
            l1=0d0 !l1=ieee_value(0._dp, ieee_quiet_nan)
            r1=0d0 !r1=ieee_value(0._dp, ieee_quiet_nan)
          endif

c-----------------------------------------------------------------------

        elseif ((nproc == 302) .or. (nproc == 307) .or. (nproc == 3002)) then
          kcase=kZgajet
          new_pspace = .true.
          nqcdjets=1
          ndim=10
...
c-- 307 '  f(p1)+f(p2) --> Z^0(-->nu(p3)+nu~(p4))+gamma(p5)+f(p6)'
          ...
          elseif (nproc == 3002) then
c-- 3002 '  f(p1)+f(p2) --> Z(-->3*(d(p3)+d~(p4)))+gamma(p5)+f(p6)'
            nqcdjets=1
            !plabel(3)='bq'
            !plabel(4)='ba'
            ! handle them for all purposes like leptons, but only for the
            ...
            call setDecayChannel(decayQuarks)
            if (removebr) then
              write (*,*) "can't remove this branching ratio"
              write(6,*) 'Abort in chooser'
              stop
              !plabel(3)='ig'
              !plabel(4)='ig'
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc == 304) .or. (nproc == 309) .or. (nproc == 3004)) then
...
c--- total cross-section
        ...
        elseif (nproc == 309) then
           ...
           l1=ln*sqrt(3._dp)
           r1=rn*sqrt(3._dp)
        elseif (nproc == 3004) then
c-- 3004 '  f(p1)+f(p2) --> Z(-->3*(d(p3)+d~(p4)))+gamma(p5)+f(p6)+f(p7)'
            plabel(3)='bq'
            plabel(4)='ba'
            q1=Q(1)*sqrt(3._dp*xn)
            l1=l(1)*sqrt(3._dp*xn)
...
c--- include singly resonant diagrams if zerowidth=.false.
        srdiags=(zerowidth .eqv. .false.)
        ...
        if    ((nproc == 461) .or. (nproc == 462)) then
c-- 461 '  f(p1)+f(p2) --> W^+(-->nu(p3)+e^+(p4)) +W^-(-->e^-(p5)+nu~(p6)) + f(p7)'
c--     '  f(p1)+f(p2) --> W^+ + W^- + jet(for total Xsect)' (removebr=.true.)
          plabel(3)='nl'
          plabel(4)='ma'
          plabel(5)='el'
          plabel(6)='na'
...
c--- total cross-section
          if (removebr) then
            ...
            call branch(brwen,brzee,brznn,brtau,brtop,brcharm)
c Adjust BR for use of CMS
            BrnRat=(brwen*xw/abs(zxw))**2
          endif
        endif

c-----------------------------------------------------------------------

      elseif ((nproc >= 470) .and. (nproc <= 480)) then
        kcase=kWZ_jet
        call checkminzmass(2)
        call readcoup
        plabel(7)='pp'
...
c Remove virtual photon
              q1=0._dp
c Adjust BR for use of CMS
              BrnRat=(brwen*xw/abs(zxw))*(brzee*(abs(zle)**2+abs(zre)**2)/(le**2+re**2))
            endif
          endif

        elseif (nproc >= 476) then
c-- W^-Z
          nwz=-1
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc > 480) .and. (nproc <= 490)) then
        kcase=kZZ_jet
        call checkminzmass(1)
        if ((nproc == 481) .or. (nproc == 483) .or. (nproc == 490)) call checkminzmass(2)
        call readcoup
        plabel(7)='pp'
        plabel(8)='pp'
        nqcdjets=1
...
c--- only include singly-resonant diagrams when not in zerowidth approx.
        ...
        if (nproc == 481 .or. nproc == 486) then
c--  81 '  f(p1)+f(p2) --> Z^0(-->e^-(p3)+e^+(p4)) + Z^0(-->mu^-(p5)+mu^+(p6))'
c--     '  f(p1)+f(p2) --> Z^0 + Z^0 (for total Xsect)' (removebr=.true.)
c--  86 '  f(p1)+f(p2) --> Z^0(-->e^-(p5)+e^+(p6))+Z^0(-->mu^-(p3)+mu^+(p4)) (NO GAMMA*)'
c--     '  f(p1)+f(p2) --> Z^0 + Z^0 (for total Xsect) (NO GAMMA*)' (removebr=.true.)
...
c Adjust BR for use of CMS
        ...
        elseif (nproc == 482 .or. nproc == 487) then
c--  82 '  f(p1)+f(p2) --> Z^0(-->e^-(p3)+e^+(p4)) + Z^0(-->3*(nu(p5)+nu~(p6)))'
c--  87 '  f(p1)+f(p2) --> Z^0(-->e^-(p3)+e^+(p4)) + Z^0(-->3*(nu(p5)+nu~(p6))) [no gamma^*]'
          plabel(3)='el'
          plabel(4)='ea'
...
c-- remove gamma^* if necessary
        if ((nproc > 485) .and. (nproc < 490)) then
          q1=zip
          q2=zip
        endif
...
c-----------------------------------------------------------------------
      ...
      elseif ((nproc >= 609) .and. (nproc <= 619)) then
        kcase=kWH1jet
        call sethparams(br,wwbr,zzbr,tautaubr,gamgambr,zgambr)
        nqcdjets=1
        plabel(7)='pp'
...
c     800 '  f(p1)+f(p2) --> V-->(X(p3)+X~(p4)) +f(p5) [Vector Mediator] ' 'N '
c     801 '  f(p1)+f(p2) --> A-->(X(p3)+X~(p4)) +f(p5) [Axial Vector Mediator] ' 'N '
c     802 '  f(p1)+f(p2) --> S-->(X(p3)+X~(p4)) +f(p5) [Scalar Mediator] ' 'N '
c     803 '  f(p1)+f(p2) --> PS-->(X(p3)+X~(p4)) +f(p5) [Pseudo Scalar Mediator] ' 'N '
c     804 '  f(p1)+f(p2) --> GG-->(X(p3)+X~(p4)) +f(p5) [Gluonic DM operator] ' 'N '
c     805 '  f(p1)+f(p2) --> S--(X(p3)+X~(p4)) +f(p5) [Scalar Mediator, mt loops] ' 'L'
         kcase=kdm_jet
         plabel(3)='xm'
         plabel(4)='xa'
         plabel(5)='pp'
         ...
         error stop "Please use MCFM-6.6 for this process"
         !call read_dm_params()
c------- phase space setup
         n3=0
         mass3=medmass
...
c     820 '  f(p1)+f(p2) --> V-->(X(p3)+X~(p4)) +gamma(p5) [Vector Mediator] ' 'F '
c     821 '  f(p1)+f(p2) --> A-->(X(p3)+X~(p4)) +gamma(p5) [Axial Vector Mediator] ' 'F '
c     822 '  f(p1)+f(p2) --> S-->(X(p3)+X~(p4)) +gamma(p5) [Scalar Mediator] ' 'F '
c     823 '  f(p1)+f(p2) --> PS-->(X(p3)+X~(p4)) +gamma(p5) [Pseudo Scalar Mediator] ' 'F '
         kcase=kdm_gam
         plabel(3)='xm'
         plabel(4)='xa'
         plabel(5)='ga'
...
c     840 '  f(p1)+f(p2) --> V-->(X(p3)+X~(p4)) +f(p5)+f(p6) [Vector Mediator] ' 'L '
c     841 '  f(p1)+f(p2) --> A-->(X(p3)+X~(p4)) +f(p5)+f(p6) [Axial Vector Mediator] ' 'L '
c     842 '  f(p1)+f(p2) --> S-->(X(p3)+X~(p4)) +f(p5)+f(p6) [Scalar Mediator] ' 'L '
c     843 '  f(p1)+f(p2) --> PS-->(X(p3)+X~(p4)) +f(p5)+f(p6) [Pseudo Scalar Mediator] ' 'L '
c     844 '  f(p1)+f(p2) --> GG-->(X(p3)+X~(p4)) +f(p5)+f(p6) [Gluonic DM operator] ' 'L '
         kcase=kdm2jet
         plabel(3)='xm'
         plabel(4)='xa'
         plabel(5)='pp'
...
c     845 '  f(p1)+f(p2) --> V-->(X(p3)+X~(p4)) +gamma(p5)+f(p6) [Vector Mediator] ' 'L '
c     846 '  f(p1)+f(p2) --> A-->(X(p3)+X~(p4)) +gamma(p5)+f(p6) [Axial Vector Mediator] ' 'L '
c     847 '  f(p1)+f(p2) --> S-->(X(p3)+X~(p4)) +gamma(p5)+f(p6) [Scalar Mediator] ' 'L '
c     848 '  f(p1)+f(p2) --> PS-->(X(p3)+X~(p4)) +gamma(p5)+f(p6) [Pseudo Scalar Mediator] ' 'L '
         kcase=kdm_gaj
         plabel(3)='xm'
         plabel(4)='xa'
         plabel(5)='ga'
