...
c  0 -> H + g(i1) + g(i2) + g(i3) + g(i4)
c with full top, bottom mass dependence retained
c Note that the bottom quark is included with the pole mass in the
c propagator,
c and a running mass mb(mh) in the Yukawa coupling
      ...
      include 'Inc/Cred.f'
      include 'Inc/I5to4.f'
      integer j,c1,c2,c3,c4,pppm,ppmm,h1,h2,h3,h4,iq
      real(dp)::mtsq,mt,mb,mt_yuk,mb_yuk,as,vevsq
      real(dp):: msq
      complex(dp) :: Dcoeff(dmax),Ccoeff(cmax),Bcoeff(bmax),Rat,
     & amp(2,2,2,2,3),ampt(2,2,2,2,3),ampb(2,2,2,2,3)
