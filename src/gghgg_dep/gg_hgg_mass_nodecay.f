...
!
      subroutine gg_hgg_mass_nodecay(p,iglue1,iglue2,msq)
...
c   Including the effect of the finite top-quark mass
      include 'masses.f'
...
c--- four gluon terms
      ...
      call hgggg_mass(za,zb,Hgggg)
      endif

c--- four quark terms
      if ((useblha==0).or.(blhatype>=7)) then
      call haqaq_mass(1,3,2,4,za,zb,Hqrqr,Hqqqq)
      Habab=Hqrqr
      Haaaa=Hqqqq
      Hqbqb=Hqrqr
      Hbqbq=Hqrqr
      ...
      if (useblha==0) then
      call haqaq_mass(1,2,4,3,za,zb,Hqarb,Hqaqa)
      Haqbr=Hqarb
      Haqaq=Hqaqa
      endif
...
c--- two quark two gluon terms
      if (useblha==0) then
      call haqgg_mass(1,2,3,4,za,zb,Hqagg)
      Haqgg=Hqagg
      endif

      if (useblha==0) then
      call haqgg_mass(1,3,2,4,za,zb,Hqgqg)
      Hagag=Hqgqg
      endif

      if (useblha==0) then
      call haqgg_mass(2,3,1,4,za,zb,Hgqqg)
      Hgaag=Hgqqg
      endif

      if ((useblha==0).or.(blhatype==2)) then
      call haqgg_mass(4,3,1,2,za,zb,Hggqa)
      endif

c      write(6,*) 'Hgggg',avegg*half*Hgggg
c      write(6,*)
...
c Print-outs useful for OpenLoops comparison
c Over-write for testing stability
c      msq(0,0)=avegg*half*Hgggg
c      msq(1,0)=aveqq*half*Hqagg
c      msq(2,0)=aveqq*Hqaqa
