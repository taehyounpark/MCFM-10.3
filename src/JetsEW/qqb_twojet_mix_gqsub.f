...
!
      subroutine qqb_twojet_mix_gqsub(p,msq)
...
c---    2.  For annihilation contribution, does not sum over final state
      implicit none
      include 'types.f'
      include 'mxpart.f'
      include 'nf.f'
     ...
     &     aqii_jj(0:3,nf,nf),aqij_ij(0:3,nf,nf)
      integer j,k
...
c --- qa
            if((j > 0) .and. (k < 0)) then
               if(j == -k) then
c                  msq_mix(:,j,k) = qaii_jj(:,j,1) + qaii_jj(:,j,2)
c     &                 + qaii_jj(:,j,3) + qaii_jj(:,j,4)
                  msq_mix(:,j,k) = qaii_jj(:,j,j)
               else
...
c --- aq
               ...
               if(k == -j) then
c                  msq_mix(:,j,k) = aqii_jj(:,k,1) + aqii_jj(:,k,2)
c     &                 + aqii_jj(:,k,3) + aqii_jj(:,k,4)
                  msq_mix(:,j,k) = aqii_jj(:,k,k)
               else
