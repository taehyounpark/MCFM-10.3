...
c--- and by type of contribution qqb(1) ... qqb(n)
      ...
      if (first) then
        first=.false.
c        if ((Gflag) .or. (QandGflag)) then
c          write(*,*) 'Using QQGG matrix elements'
c          write(*,*) '[LC is   N ]'
c          write(*,*) '[SLC is 1/N]'
c        endif
c        if ((Qflag) .or. (QandGflag)) then
c          write(*,*) 'Using QQBQQB matrix elements'
c          write(*,*) '[LC is   1 ]'
c          write(*,*) '[SLC is 1/N]'
c        endif
cc        if (kpart==klord) then
c          if     (colourchoice == 1) then
c            write(*,*) 'Leading colour only'
c          elseif (colourchoice == 2) then
c            write(*,*) 'Sub-leading colour only'
c          elseif (colourchoice == 0) then
c            write(*,*) 'Total of both colour structures'
c          else
c            write(*,*) 'Bad colourchoice'
c            stop
c          endif
cc        else
cc          write(*,*) 'Calculating all colour structures in LO'
cc        endif
      endif
...
c--- QBAR QBAR - different flavours
           if (useblha==1) then
              if (blhafl(5)==blhafl(6)) then
          mqq(0,j,k)=facqq*half*Vsq(k,-j)*qbqb_ijjj(0)
          mqq(1,j,k)=facqq*half*Vsq(k,-j)*qbqb_ijjj(1)
          mqq(2,j,k)=facqq*half*Vsq(k,-j)*qbqb_ijjj(2)
              else
          mqq(1,j,k)=facqq*Vsq(-blhafl(6),k)*qbqb_ijik(1)
              endif
          else
          mqq(0,j,k)=
...
c--- QBAR QBAR - same flavours
           if (useblha==1) then
          mqq(1,j,k)=facqq*Vsq(-blhafl(6),k)*qbqb_iiji(1)
           else
