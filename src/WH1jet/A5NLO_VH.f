...
!
      ...
      subroutine A5NLO_VH(j1,j2,j3,j4,j5,za,zb,includeaxial,A5LOm,A5NLOm,A5ax)
      implicit none
      include 'types.f'
      include 'constants.f'
      include 'mxpart.f'
      ...
      complex(dp):: A51_VH,A52_VH,A53_VH,A5NLOm,A5LOm,A5ax,zab2
      real(dp):: s125,mt2
...
c      A5LOm=-zb(j1,j4)**2/(zb(j2,j5)*zb(j5,j1)*zb(j4,j3))
     ...
     &     +A52_VH(j2,j1,j5,j4,j3,zb,za)/xnsq
      ...
      if (includeaxial) then
        mt2=mt**2
        A5ax=A53_VH(j2,j1,j5,j4,j3,zb,za,mt2)/xn
      endif
