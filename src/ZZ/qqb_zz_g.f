...
c---    2nd pass --> fill msq
        if (ii == 1) then
          ...
          if (h34 == h56) then
            msq(j,k)=msq(j,k)+fac1*ave*(abs(Uncrossed(j,k,hq,h34,h56,hg)-amp)**2
     &                             +abs(Uncrossed(j,k,hq,h34,h56,hg))**2-abs(amp)**2)
          else
            msq(j,k)=msq(j,k)+fac1*ave*(2._dp*abs(Uncrossed(j,k,hq,h34,h56,hg))**2)
          endif
        endif
      endif
