...
c---  calculate the qg and qbarg matrix element first
               call interdk(p,1,2,3,4,6,7,8,mt,mb,msq_qg)
               call interdk(p,7,2,3,4,6,1,8,mt,mb,msq_qbarg)
c---  now calculate the gq and gqbar matrix element
               call interdk(p,2,1,3,4,6,7,8,mt,mb,msq_gq)
               call interdk(p,7,1,3,4,6,2,8,mt,mb,msq_gqbar)
            else
c---  calculate the qg and qbarg matrix element first
               call interdk(p,1,2,4,3,6,7,8,mb,mt,msq_qg)
               call interdk(p,7,2,4,3,6,1,8,mb,mt,msq_qbarg)
c---  now calculate the gq and gqbar matrix element
               call interdk(p,2,1,4,3,6,7,8,mb,mt,msq_gq)
               call interdk(p,7,1,4,3,6,2,8,mb,mt,msq_gqbar)
            endif
         endif
...
c--- the matrix elements with an extra quark line:
         if (nwz == +1) then
            call interdk_qq(p,1,2,3,4,6,7,8,mt,mb,msq_qq12,msq_qq12_intf)
            call interdk_qq(p,7,8,3,4,6,1,2,mt,mb,msq_qq56,msq_qq56_intf)
         else
            call interdk_qq(p,1,2,4,3,6,7,8,mb,mt,msq_qq12,msq_qq12_intf)
            call interdk_qq(p,7,8,4,3,6,1,2,mb,mt,msq_qq56,msq_qq56_intf)
         endif
      endif
