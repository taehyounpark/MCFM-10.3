...
module nplotter_Higgs
      ...
      subroutine setup()
          ...
          histos(3) = plot_setup_custom([0d0,15d0,30d0,45d0,80d0,120d0, &
              200d0,350d0,600d0,14000d0],'pt34_cms')
      ...
      subroutine book(p,wt,ids,vals,wts)
          ...
          real(dp) :: pttwo,twomass,pt34, trans
          real(dp) :: pparton(mxpart,4)


          call getptilde(0,pparton)

          pt34 = pttwo(3,4,pparton)

          if (origKpart == kresummed) then
              if (abovecut .eqv. .false.) then
                  trans = transition((pt34/twomass(3,4,pparton))**2d0,0.001d0,transitionSwitch,0.001d0)
              else
          ...
          vals = [pt34,pt34,pt34]
          wts = [wt*trans,wt*trans,wt*trans]
