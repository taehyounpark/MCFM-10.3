...
module nplotter_Higgs_to_Zga
      ...
      subroutine book(p,wt,ids,vals,wts)
          ...
          ptZga = ptthree(3,4,5,pparton)

          if (origKpart == kresummed) then
              if (abovecut .eqv. .false.) then
                  trans = transition((ptZga/threemass(3,4,5,pparton))**2d0,0.001d0,transitionSwitch,0.001d0)
              else
