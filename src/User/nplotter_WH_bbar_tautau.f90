...
module nplotter_WH_bbar_tautau
      use types
      use MCFMPlotting
      ...
      subroutine book(p,wt,ids,vals,wts)
          ...
          real(dp) :: ptfour,fourmass

          real(dp) :: pparton(mxpart,4)
          real(dp) :: ptWH, trans

          call getptilde(currentNd,pparton)
          ptWH = ptfour(3,4,5,6,pparton)

          if (origKpart == kresummed) then
              if (abovecut .eqv. .false.) then
                  trans = transition((ptWH/fourmass(3,4,5,6,pparton))**2d0,0.001d0,transitionSwitch,0.001d0)
              else
