...
!
      ...
      implicit none
      include 'types.f'
c     CW----- this is copy of the Z+2j version
c------ changes 5=>4 6=>5
c***********************************************************************
c     Author: J.M. Campbell                                            *
c     September, 2001.                                                 *
c     Additions Aug. 2001, for 4Q piece                                *
c***********************************************************************
c                                                                      *
c     The value of COLOURCHOICE determines which colour structures     *
c     are included in the terms for the QQGG piece                     *
c                                                                      *
c***********************************************************************
      include 'constants.f'
      include 'mxpart.f'
...
c       xl15=log(-two*dot(p,1,5)/musq)
c       xl16=log(-two*dot(p,1,4)/musq)
c       xl25=log(-two*dot(p,2,5)/musq)
c       xl26=log(-two*dot(p,2,4)/musq)
c       xl56=log(+two*dot(p,4,5)/musq)


c***********************************************************************
c     Contributions from QQGG matrix elements                          *
c***********************************************************************
      if (Gflag) then
c--- QUARK-ANTIQUARK contributions
      if ((colourchoice == 1) .or. (colourchoice == 0)) then
      do is=1,3
      R1(q,q,a,1,is)=ason4pi*xn*(if_qq(z,xl15,is)+fi_gg(z,xl15,is)/2._dp+fi_gq(z,xl15,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      R1(q,q,a,2,is)=ason4pi*xn*(if_qq(z,xl16,is)+fi_gg(z,xl16,is)/2._dp-fi_gq(z,xl16,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      R2(a,a,q,1,is)=ason4pi*xn*(if_qq(z,xl26,is)+fi_gg(z,xl26,is)/2._dp-fi_gq(z,xl26,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      R2(a,a,q,2,is)=ason4pi*xn*(if_qq(z,xl25,is)+fi_gg(z,xl25,is)/2._dp+fi_gq(z,xl25,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      enddo
      endif
     ...
     &  (if_qq(z,xl15,is)+fi_gg(z,xl15,is)/2._dp+fi_gq(z,xl15,is)/2._dp
     &  +if_qq(z,xl16,is)+fi_gg(z,xl16,is)/2._dp-fi_gq(z,xl16,is)/2._dp)
     ...
     &  (if_qq(z,xl25,is)+fi_gg(z,xl25,is)/2._dp+fi_gq(z,xl25,is)/2._dp
     &  +if_qq(z,xl26,is)+fi_gg(z,xl26,is)/2._dp-fi_gq(z,xl26,is)/2._dp)
...
c--- ason4pi*xn*ff_gg(z,xl56,2)*( (1) + (2) )
      if ((colourchoice == 1) .or. (colourchoice == 0)) then
      do is=1,3
      R1(a,a,q,1,is)=ason4pi*xn*(if_qq(z,xl16,is)+fi_gg(z,xl16,is)/2._dp-fi_gq(z,xl16,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      R1(a,a,q,2,is)=ason4pi*xn*(if_qq(z,xl15,is)+fi_gg(z,xl15,is)/2._dp+fi_gq(z,xl15,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      R2(q,q,a,1,is)=ason4pi*xn*(if_qq(z,xl25,is)+fi_gg(z,xl25,is)/2._dp+fi_gq(z,xl25,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      R2(q,q,a,2,is)=ason4pi*xn*(if_qq(z,xl26,is)+fi_gg(z,xl26,is)/2._dp-fi_gq(z,xl26,is)/2._dp
     &                          +ff_gg(z,xl56,is)/2._dp)
      enddo
      endif
      if ((colourchoice == 2) .or. (colourchoice == 0)) then
...
c***********************************************************************
c     Contributions from QQQQ matrix elements                          *
c***********************************************************************
      if (Qflag) then
