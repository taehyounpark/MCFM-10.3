...
c---define modified (zero-mass) vectors
      ...
      elseif (j==5) then
         p12(j,nu)=p(j,nu)-0.5_dp*mc**2*p(2,nu)/p2Dp(5)
         p21(j,nu)=p(j,nu)-0.5_dp*mc**2*p(1,nu)/p1Dp(5)
      elseif (j==6) then
         p12(j,nu)=p(j,nu)-0.5_dp*mc**2*p(1,nu)/p1Dp(6)
         p21(j,nu)=p(j,nu)-0.5_dp*mc**2*p(2,nu)/p2Dp(6)
      endif
      enddo
      enddo
