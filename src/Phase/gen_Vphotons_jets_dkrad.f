...
c----
      ...
      real(dp):: p45(4),p4(4),p5(4),wt45,wt345,xjac,yave,ymax
...
c--- total number of photons and jets generated outside of boson decay
      ...
      if (nphotsjets == 0) then
        goto 77 ! no loop to perform (nphots=1, njets=0)
      endif
...
c          write(6,*) 'etamax**2 <= 1._dp in gen_phots_jets.f',etamax**2
          wt=0._dp
          return 1
        endif
...
c--- now decay Q into 3,4,5
      call phi1_2m_nobw(zip,r(3*nphotsjets+3),r(3*nphotsjets+4),
...
c      do nu=1,4
c      if (r(3*nphotsjets+8) < 0.5_dp) then
c        p(3,nu)=p3(nu)
c        p(4,nu)=p4(nu)
c      else
c        p(3,nu)=p4(nu)
