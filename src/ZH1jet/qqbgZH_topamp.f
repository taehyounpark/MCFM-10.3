...
!
      function qqbgZH_topamp(i1,i2,i3,i4,i5,za,zb,mt2)
...
c Propagator should be applied outside this routine
      prop34=cone

c====== if
      if(useeft_wh) then
      fac=-im*sqrt(gsq)*abs(zesq)*prop34*as/(3._dp*pi*sqrt(vevsq))*rt2
      else
      fac=im*sqrt(gsq)*ason4pi*abs(zesq)*prop34*mt2/sqrt(wmass**2-im*wmass*wwidth)*gw/rt2/2._dp
      endif
...
c===== total
      qqbgZH_topamp=helpart*intfunc*fac
