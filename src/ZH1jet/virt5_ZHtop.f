...
c   0--> qb_R(1)+q_L(2)+l_L(3)+a_R(4)+g_L(5)
      call A5NLO_ZHtop(ip(2),ip(1),ip(3),ip(4),ip(5),za,zb,A5LOp,A5NLOp)
c   0--> qb_R(1)+q_L(2)+l_L(3)+a_R(4)+g_R(5)
      call A5NLO_ZHtop(ip(1),ip(2),ip(4),ip(3),ip(5),zb,za,A5LOm,A5NLOm)

      virt5_ZHtop=real(conjg(A5LOp)*A5NLOp+conjg(A5LOm)*A5NLOm,kind=dp)
      return
      end
