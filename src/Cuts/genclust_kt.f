...
!
      ...
      subroutine genclust_kt(q,Rmin,qfinal,isub,ipow,nd)
      use types
      use jettagging
...
c---                    ipow = +1 (normal kt), ipow = -1 ("anti-kt")
      ...
      real(dp), intent(in) :: q(mxpart,4), Rmin
      real(dp), intent(out) :: qfinal(mxpart,4)
      integer, intent(in) :: isub,ipow,nd

      real(dp):: qjet(mxpart,4)
      real(dp):: pt,dijmin,dkmin,ayrap,aetarap,pti,ayi
      integer:: i,nu,iter,nmin1,nmin2,maxjet,nk,
     & ajet,jetindex(mxpart)
      logical:: jetmerge,failed,is_hadronic
      common/jetmerge/jetmerge
!$omp threadprivate(/jetmerge/)

      integer :: jetcontent_tmp(mxpart)
...
c--- the dipole contributions, where isub=1.
      ...
      if (is_hadronic(i)) then
        ...
        jetcontent(maxjet,currentNd) = bitflag(i)
      endif
      enddo
...
c        write(*,*) 'Clustered ',nmin1,nmin2
        ...
        jetcontent(nmin1,currentNd) = jetcontent(nmin1,currentNd) +
     &                                jetcontent(nmin2,currentNd)
...
c--- so if this condition isn't true then there's one jet left at maxjet
 ...
 2    continue
      ...
      ! get same ordering in jetcontent as in pjet
      jetcontent_tmp(:) = jetcontent(:,currentNd)
      jetcontent(:,currentNd) = 0

c--- restore incoming partons
      qfinal(1:2,:)=q(1:2,:)
      jetcontent(1:2,currentNd) = bitflag(1:2)
c--- set all other momenta to zero and restore leptons
      do i=3,npart+2
        qfinal(i,:) = 0._dp
        if (.not.(is_hadronic(i))) then
          qfinal(i,:)=q(i,:)
          jetcontent(i,currentNd) = bitflag(i)
        endif
      enddo
...
c        write(*,*) 'aeta: ',aetarap(i,qjet),' vs max. ',etajetmax
     ...
     &      (ayi >= etajetmin) .and. (ayi <= etajetmax))
     &     .or. (jetptveto < 1.e4_dp) .or. (usept)) then
          ajet=ajet+1
          qfinal(jetindex(ajet),:)=qjet(i,:)
          jetlabel(ajet)=jetlabel(i)

          jetcontent(jetindex(ajet),currentNd) = jetcontent_tmp(i)
        endif
      enddo
...
c--- if no jets are removed by eta and pt cuts, then jets=ajet
      if (ajet < jets) then
        do i=ajet+1,jets
          qfinal(jetindex(i),:) = 0._dp
        enddo
        jets=ajet
      endif

c-- check jets for heavy quark content and invariant mass
      if (nproc /= 1610 .and. nproc /= 1650) then
