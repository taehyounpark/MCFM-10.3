...
extern "C" {
  void qqb_gmgmjt_(double *p,double *msq);
  void qqb_gmgmjt_v_(double *p,double *msqv);
}

namespace MCFM {

  class qqb_gmgmjt: public Process {
  public:
	...
	if (legs[i].Mass()) return false;
      ...
      if (legs[1].m_fl!=22) return false;
      if (legs[3].m_fl>0 && legs[3].m_fl<6 &&
	  legs[4].m_fl==-legs[3].m_fl && legs[2].m_fl==21)
	return interface->AddProcess(pi,new qqb_gmgmjt(legs))>-1;
      return false;
    }

    qqb_gmgmjt(const std::vector<Leg> &legs):
      Process(legs,4,3) {
