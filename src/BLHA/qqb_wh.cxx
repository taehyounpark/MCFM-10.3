...
extern "C" {
  void qqb_wh_gaga_v_(double *p,double *msqv);
  void qqb_wh_gaga_(double *p,double *msqv);}

namespace MCFM {

  class qqb_wh: public Process {
  private:
  ...
  public:
	...
	return interface->AddProcess(pi,new qqb_wh(legs,1))>-1;
      if (legs[0].m_fl==25 &&
	  legs[1].m_fl>10 && legs[1].m_fl<17 &&
	  legs[2].m_fl==-legs[1].m_fl+1 &&
	  legs[3].m_fl>0 && legs[3].m_fl<6 &&
	  legs[4].m_fl==-legs[3].m_fl-1)
	return interface->AddProcess(pi,new qqb_wh(legs,2))>-1;
      return false;
    }

    qqb_wh(const std::vector<Leg> &legs,int type):
      Process(legs,4,3), m_type(type) {
      static int first(true);
      if (first) {
	first=false;
	nproc_.nproc=(legs[1].m_fl%2)?98:93;
	blha_.useblha=1;
	chooser_();
        m_hdecaymode="none";
    ...
    void Calc(const std::vector<FourVec> &p,int oqcd) {
      ...
      SetMom(p_p,p,m_legs[1],2);// FS e- / v
      SetMom(p_p,p,m_legs[2],3);// FS vb / e+
      SetMom(p_p,p,m_legs[0],4);// FS h
      // Calculate result.
      epinv_.epinv=epinv2_.epinv2=0.0;
      qqb_wh_gaga_v_(p_p,p_msqv);
