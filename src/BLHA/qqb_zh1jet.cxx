...
extern "C" {
  void qqb_zh1jet_(double *p,double *msq);
  void qqb_zh1jet_v_(double *p,double *msqv);
}

namespace MCFM {

  class qqb_zh1jet: public Process {
  private:
  ...
  public:
	...
	if (legs[i].Mass()) return false;
      if (legs[0].m_fl!=25) return false;
      if (!(legs[1].m_fl>10 && legs[1].m_fl<17 &&
	    legs[2].m_fl==-legs[1].m_fl)) return false;
      if (legs[4].m_fl>0 && legs[4].m_fl<6 &&
	  legs[5].m_fl==-legs[4].m_fl && legs[3].m_fl==21)
	return interface->AddProcess(pi,new qqb_zh1jet(legs))>-1;
      return false;
    }
    ...
    qqb_zh1jet(const std::vector<Leg> &legs):
      Process(legs,5,4), m_id{5,4,1,2,0,3},
      m_ccfac((legs[1].m_fl%2)?1.:3.) {
    ...
    void Calc(const std::vector<FourVec> &p,int oqcd) {
      ...
      SetMom(p_p,p,m_legs[m_id[2]],2);// FS e-
      SetMom(p_p,p,m_legs[m_id[3]],3);// FS e+
