...
extern "C" { void gg_hg_mass_(double *p,int *iglue,double *msqv); }

namespace MCFM {

  class gg_hg_mass: public Process {
  private:
  ...
  public:
	...
	return interface->AddProcess(pi,new gg_hg_mass(legs,0))>-1;
      if (legs[2].m_fl>0 && legs[2].m_fl<6 &&
	  legs[3].m_fl==-legs[2].m_fl && legs[1].m_fl==21)
	return interface->AddProcess(pi,new gg_hg_mass(legs,1))>-1;
      return false;
    }

    gg_hg_mass(const std::vector<Leg> &legs,int type):
    ...
    void Calc(const std::vector<FourVec> &p,int oqcd) {
      ...
      // Calculate result.
      blha_.blhatype=1+m_type*m_id3;
      gg_hg_mass_(p_p,&m_iglue,p_msqv);
