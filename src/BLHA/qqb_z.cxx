...
namespace MCFM {
  ...
  public:
	...
	if (legs[i].Mass()) return false;
      if (legs[0].m_fl>10 && legs[0].m_fl<17 && legs[1].m_fl==-legs[0].m_fl &&
	  legs[2].m_fl>0 && legs[2].m_fl<6 && legs[3].m_fl==-legs[2].m_fl)
        return interface->AddProcess(pi,new qqb_z(legs))>-1;
      return false;
    }

    qqb_z(const std::vector<Leg> &legs):
      Process(legs,3,2), m_ccfac((legs[0].m_fl%2)?1.:3.) {
      static int first(true);
      if (first) {
	first=false;
        nproc_.nproc=(legs[0].m_fl%2)?31:32;
	blha_.useblha=1;
	chooser_();
	GetCouplings();
      }
