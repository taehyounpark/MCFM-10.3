...
extern "C" {
  void qqb_wgamg_(double *p,double *msq);
  void qqb_wgamg_v_(double *p,double *msqv);
}

namespace MCFM {

  class qqb_wgamg: public Process {
  private:
  ...
  public:
	...
	if (legs[i].Mass()) return false;
	  ...
	  legs[5].m_fl==-legs[4].m_fl+1 &&
          legs[3].m_fl==21)
	return interface->AddProcess(pi,new qqb_wgamg(legs,1))>-1;
      if (legs[0].m_fl==22 &&
	  legs[1].m_fl>10 && legs[1].m_fl<17 &&
	  legs[2].m_fl==-legs[1].m_fl+1 &&
	  legs[4].m_fl>0 && legs[4].m_fl<6 &&
	  legs[5].m_fl==-legs[4].m_fl-1 &&
          legs[3].m_fl==21)
	return interface->AddProcess(pi,new qqb_wgamg(legs,2))>-1;
      return false;
    }

    qqb_wgamg(const std::vector<Leg> &legs,int type):
      Process(legs,5,4), m_type(type) {
      static int first(true);
      if (first) {
	first=false;
	nproc_.nproc=(type%2)?297:292;
	blha_.useblha=1;
	chooser_();
      }
      m_res.resize(4);
