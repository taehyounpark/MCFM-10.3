...
extern "C" {
  void qqb_wwg_v_(double *p,double *msqv);
  void qqb_wwg_(double *p,double *msqv);}

namespace MCFM {

  class qqb_wwg: public Process {
  private:
  ...
  public:
	...
	return interface->AddProcess(pi,new qqb_wwg(legs,1))>-1;
      if (legs[0].m_fl>10 && legs[0].m_fl<17 &&
	  legs[2].m_fl==-legs[0].m_fl-1 &&
          legs[1].m_fl>10 && legs[1].m_fl<17 &&
	  legs[3].m_fl==-legs[1].m_fl+1 &&
	  legs[5].m_fl>0 && legs[5].m_fl<6 && legs[6].m_fl==-legs[5].m_fl &&
  	  legs[4].m_fl==21  &&
	  CheckDecays(pi,legs,0,2,1,3))
	return interface->AddProcess(pi,new qqb_wwg(legs,2))>-1;
      if (legs[0].m_fl>10 && legs[0].m_fl<17 &&
    ...
    qqb_wwg(const std::vector<Leg> &legs,int type):
