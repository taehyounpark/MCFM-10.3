...
namespace MCFM {
  ...
  class qqb_z2jet: public Process {
  private:
  ...
  public:
	...
	if (legs[4].m_fl>0 && legs[4].m_fl<6 && legs[5].m_fl==-legs[4].m_fl)// ggqa
	  return interface->AddProcess(pi,new qqb_z2jet(legs,1))>-1;
      }
      if (legs[2].m_fl>0 && legs[2].m_fl<6 && legs[4].m_fl==-legs[2].m_fl) {
       	if (legs[3].m_fl==legs[2].m_fl && legs[5].m_fl==-legs[3].m_fl)// aaaa
	  return interface->AddProcess(pi,new qqb_z2jet(legs,2))>-1;
	if (legs[3].m_fl>0 && legs[3].m_fl<6 && legs[5].m_fl==-legs[3].m_fl)// qrqr
	  return interface->AddProcess(pi,new qqb_z2jet(legs,3))>-1;
      }
      return false;
    }

    qqb_z2jet(const std::vector<Leg> &legs,int type):
      Process(legs,2,3), m_type(type),
      m_ccfac((legs[0].m_fl%2)?1.:3.) {
      static int first(true);
      if (first) {
	first=false;
        nproc_.nproc=(legs[0].m_fl%2)?44:46;
	blha_.useblha=1;
	chooser_();
	GetCouplings();
      }
