...
namespace MCFM {
  ...
  public:
	...
	return interface->AddProcess(pi,new qqb_zgam(legs))>-1;
      return false;
    }

    qqb_zgam(const std::vector<Leg> &legs):
      Process(legs,4,3), m_ccfac((legs[1].m_fl%2)?1.:3.) {
      static int first(true);
      if (first) {
	first=false;
	nproc_.nproc=(legs[1].m_fl%2)?300:305;
	blha_.useblha=1;
	chooser_();
      }
      m_res.resize(4);
