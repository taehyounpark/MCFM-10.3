...
extern "C" {
  void qqb_fourgam_(double *p,double *msq);
  void qqb_fourgam_v_(double *p,double *msqv);
}

namespace MCFM {

  class qqb_fourgam: public Process {
  public:
	...
	if (legs[i].Mass()) return false;
	  ...
	  legs[2].m_fl!=22 || legs[3].m_fl!=22) return false;
      if (legs[4].m_fl>0 && legs[4].m_fl<6 &&
	  legs[5].m_fl==-legs[4].m_fl)
	return interface->AddProcess(pi,new qqb_fourgam(legs))>-1;
      return false;
    }

    qqb_fourgam(const std::vector<Leg> &legs):
      Process(legs,5,4) {
    ...
    void Calc(const std::vector<FourVec> &p,int oqcd) {
      ...
      SetMom(p_p,p,m_legs[2],4);// FS y
      SetMom(p_p,p,m_legs[3],5);// FS y
      // Calculate result.
      epinv_.epinv=epinv2_.epinv2=0.0;
      qqb_fourgam_v_(p_p,p_msqv);
