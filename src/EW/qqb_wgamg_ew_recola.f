...
!
      subroutine qqb_wgamg_ew_recola(pmcfm,msqnlo)
...
c Variables for this demo file
      real (dp)          :: p(0:3,1:6),MW,MZ,WW,WZ,GetDeltarr
      integer::colour(6),hel(6),j,k,ii
      complex(dp)::Anlo(2)
...
c The standard output is selected
        call set_output_file_rcl('wgamg_ew')
...
c        else
c Turning on alpha0 scheme requires deltaPR to be set
...
c+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        if (nwz==+1) then
        ip=60; i0=ip
        call define_process_rcl(ip,'u d~ -> nu_e e+ A g','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 2; kk(ip) = -1

        ip=ip+1
        call define_process_rcl(ip,'d~ u -> nu_e e+ A g','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = -1; kk(ip) = 2
        ...
        ip=ip+1
        call define_process_rcl(ip,'g u -> nu_e e+ A d','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 0; kk(ip) = 2
        ...
        ip=ip+1
        call define_process_rcl(ip,'u g -> nu_e e+ A d','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 2; kk(ip) = 0
        ...
        ip=ip+1
        call define_process_rcl(ip,'d~ g -> nu_e e+ A u~','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = -1; kk(ip) = 0
        ...
        ip=ip+1
        call define_process_rcl(ip,'g d~ -> nu_e e+ A u~','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 0; kk(ip) = -1
        ...
        elseif (nwz==-1) then
        ip=65; i0=ip
        call define_process_rcl(ip,'d u~ -> e- nu_e~ A g','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 1; kk(ip) = -2
        ...
        ip=ip+1
        call define_process_rcl(ip,'u~ d -> e- nu_e~ A g','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = -2; kk(ip) = 1
        ...
        ip=ip+1
        call define_process_rcl(ip,'g d -> e- nu_e~ A u','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 0; kk(ip) = 1
        ...
        ip=ip+1
        call define_process_rcl(ip,'d g -> e- nu_e~ A u','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 1; kk(ip) = 0
        ...
        ip=ip+1
        call define_process_rcl(ip,'u~ g -> e- nu_e~ A d~','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = -2; kk(ip) = 0
        ...
        ip=ip+1
        call define_process_rcl(ip,'g u~ -> e- nu_e~ A d~','NLO')
        call unselect_gs_power_LoopAmpl_rcl(ip,0)
        call select_gs_power_BornAmpl_rcl(ip,1)
        call select_gs_power_LoopAmpl_rcl(ip,1)
        jj(ip) = 0; kk(ip) = -2
      ...
      p(:,6)=+[pmcfm(6,4),pmcfm(6,1),pmcfm(6,2),pmcfm(6,3)]
...
c      call set_delta_ir_rcl(1d0,0d0)
      ...
      call get_squared_amplitude_rcl(ii, 1, 'LO', msqlo(jj(ii),kk(ii)))
      call get_squared_amplitude_rcl(ii, 1, 'NLO', msqnlo(jj(ii),kk(ii)))
...
c Repeat from 1st generation to 2nd
      do j=3,4
      msqlo(j,0)=msqlo(j-2,0)
      msqlo(-j,0)=msqlo(-j+2,0)
      msqlo(0,j)=msqlo(0,j-2)
      msqlo(0,-j)=msqlo(0,-j+2)
      msqnlo(j,0)=msqnlo(j-2,0)
      msqnlo(-j,0)=msqnlo(-j+2,0)
      msqnlo(0,j)=msqnlo(0,j-2)
      msqnlo(0,-j)=msqnlo(0,-j+2)
      do k=3,4
