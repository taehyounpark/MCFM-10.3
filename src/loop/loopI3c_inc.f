...
!
      real(dp):: mu2
      complex(dp):: loopI3_res,resQCDLoop,resOneLOop,result(0:2)
      integer ep
...
c--- call to QCDLoop if necessary      
      if ((scalarselect .eq. 1) .or.(scalarselect .eq. 3)) then
        resQCDLoop=qlI3c(p1,p2,p3,m1,m2,m3,mu2,ep)
      endif
      
      if (scalarselect .eq. 1) then
        loopI3_res=resQCDLoop
        return
      endif
      
      call olo_c0(result,p1,p2,p3,m1,m2,m3,sqrt(mu2))
      resOneLOop=result(abs(ep))
      loopI3_res=resOneLOop
      
      if (scalarselect .eq. 3) then
        if ((cdabs(resOneLOop) .gt. 1d-12) .and.
     &      (abs(resQCDLoop/resOneLOop-1d0) .gt. 1d-12)) then
          write(6,*) 'loopI3: ',p1,p2,p3,m1,m2,m3,mu2,ep
